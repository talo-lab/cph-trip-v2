...
// 숙소 (Airbnb · 베스테르브로, 상트요르겐스 호수 인근 · 정확 좌표 비공개로 대략 위치)
const STAY = {
  name:'우리 숙소 (Airbnb)',
  lat:55.6745, lng:12.5535,
  desc:'베스테르브로 · 상트요르겐스 호수 근처 · 코펜하겐 중앙역 900m · 현대적 아파트 (2인)',
...
const DEFAULT_PLAN = [
  ...
  {date:'6/9 (화)', tag:'코펜하겐 도착', fest:false, items:[
    ...
    {time:'08:30', title:'[미주] CPH(T2) 도착', note:'AMS 환승(KL1267) 후 도착 · 공항에서 합류', dist:'', _fixed:true},
    {time:'오전', title:'베스테르브로 숙소 짐 보관', note:'중앙역 900m · 체크인은 15:00부터', dist:''},
    {time:'15:00', title:'Airbnb 체크인 (베스테르브로)', note:'상트요르겐스 호수 근처 현대적 아파트', dist:''},
    {time:'16:00', title:'Installation', note:'Fredericiagade 57, 1310 Copenhagen K · 설치 미술 감상', dist:'', _lat:55.6844, _lng:12.5903, _fixed:true},
  ...
  {date:'6/10 (수)', tag:'페스티벌 1일차', fest:true, items:[
    {time:'17:00', title:'🍽 Food & Music with SALU (예약 완료)', note:'소셜 다이닝 3명 · Folkehuset Absalon, Sønder Blvd. 73 · 17:00–20:00 · QR코드 보유 · 숙소 도보권', dist:'고정 일정', _lat:55.665398, _lng:12.550298, _fixed:true},
  ]},
...
function renderPlanMarkers(di){
  ...
  plan[di].items.forEach((it)=>{
    ...
    const icon = L.divIcon({
      className:'',
      html:`<div class="plan-pin" style="background:${dayColor}"><span>${catIcon}</span></div>`,
      iconSize:[30,30], iconAnchor:[15,30], popupAnchor:[0,-32]
    });
    const m = L.marker([coords.lat,coords.lng],{icon}).addTo(map);
    m.bindPopup(`<div class="pop-name">${it.title}</div><div class="pop-desc">${it.time||''}${it.note?' · '+it.note:''}<br><b style="color:${dayColor}">${plan[di].date}</b></div>`);
    planPinLayer.push(m);
  });
}
...
function updateDayViz(di){
  ...
  const pts = plan[di].items.filter(it=>it._lat&&it._lng).map(it=>[it._lat,it._lng]);
  if(pts.length>=2){
...
const FEST_CATEGORIES = [
  ...
  {key:'dining',   label:'다이닝',   icon:'☕', color:'#7a4a1e'},
...
function openEvModal(ev, id){
  const existing = document.getElementById('evOverlay');
  if(existing){
    existing.remove();
    // 같은 이벤트 재클릭이면 닫기만 (토글)
...
설명: ${ev.desc}`;
  ...
  try{
    ...
    if(detailEl) detailEl.innerHTML=`<div style="font-size:12px;color:var(--slate);opacity:.7">상세 정보를 불러올 수 없어요. (${e.message})</div>`;
  }
}
...
function renderFest(){
  ...
  topBar.innerHTML=`<span style="font-size:11px;color:var(--slate)">공식 행사 데이터 · ${FESTIVAL_EVENTS.length}개 로드됨</span>
  ...
  filters.className = 'fest-filters';
  ...
  // ── 카테고리 필터 (구분선)
  const catSep = document.createElement('div'); catSep.style.cssText='width:100%;height:0'; filters.appendChild(catSep);
  ...
  // ── 지구 필터 (구분선)
  const distSep = document.createElement('div'); distSep.style.cssText='width:100%;height:0'; filters.appendChild(distSep);
  ...
  // ── 즐겨찾기 필터 버튼
  const favSep = document.createElement('div'); favSep.style.cssText='width:100%;height:0'; filters.appendChild(favSep);
  ...
  const filtered = FESTIVAL_EVENTS.filter(e=>
    e.date===festDateFilter &&
    (festCatFilter==='all' || e.category===festCatFilter) &&
    (festDistFilter==='all' || e.district===festDistFilter) &&
    (!festFavFilter || favorites.has(`fest-${e.date}-${e.title}`))
  );

  if(filtered.length===0){
    list.innerHTML = `<div style="padding:24px;text-align:center;color:var(--slate);font-size:13px">해당 날짜/지구에 등록된 행사가 없습니다</div>`;
  }
  ...
  filtered.forEach((ev)=>{
    ...
    const dkColor = districtColor(ev.district);
    const catInfo = FEST_CATEGORIES.find(c=>c.key===ev.category)||FEST_CATEGORIES[0];
    ...
    // 이미 체크된 항목은 렌더 시 마커 복원
    if(isChecked) showFestMarker(ev, id);

    // 즐겨찾기 토글
    const favBtnEl = row.querySelector('.fav-btn');
    favBtnEl.addEventListener('click', e=>{
      e.stopPropagation();
      if(favorites.has(id)){ favorites.delete(id); favBtnEl.textContent='♡'; }
      else { favorites.add(id); favBtnEl.textContent='♥'; }
      saveFavs();
    });
...
async function addFestSelected(){
  ...
  toAdd.forEach(ev=>{
    ...
    if(di==null) return;
    // 중복 방지
    if(plan[di].items.some(it=>it.title===ev.title)) return;
    plan[di].items.push({
  ...
  // 피드백 + 일정 탭 갱신 (백그라운드)
  const btn = document.getElementById('festAddBtn');
  if(btn){ btn.textContent=`✓ ${toAdd.length}개 일정에 추가됨`; btn.disabled=true; }
  // plan 인메모리는 이미 업데이트됨 → 일정 탭으로 이동 시 즉시 반영
  setTimeout(()=>{
    if(activeTab==='fest') renderFest();
...
function renderPlan(){
  ...
  plan.forEach((day,di)=>{
    ...
    day.items.forEach((it,ii)=>{
      ...
      if(it._done) row.classList.add('done');
      ...
      row.innerHTML = `
        ...
        <input type="checkbox" class="item-done-cb" ${it._done?'checked':''} title="완료 표시">
      ...
      // 완료 체크박스
      const doneCb = row.querySelector('.item-done-cb');
      doneCb.addEventListener('change', e=>{
        e.stopPropagation();
        plan[di].items[ii]._done = doneCb.checked;
        row.classList.toggle('done', doneCb.checked);
        savePlan();
      ...
      // 행 클릭 → 지도 경로 시각화 + 드로어 열기
      row.addEventListener('click', e=>{
        if(e.target.matches('[contenteditable],[contenteditable] *,.item-x,.item-done-cb')) return;
    ...
    add.addEventListener('click',()=>{
      plan[di].items.push({time:'',title:'새 일정',note:'',dist:'',_addedBy:currentUser,_personal:false,_with:['miju','sanghyo']});
      savePlan(); renderPlan();
    });
  ...
  // 진행률 바 초기화
  plan.forEach((_,di)=>updateDayProgress(di));
  // 충돌 경고 배너
...
const RECOMMEND = [
  { dayIdx:1, label:'DAY 1', date:'6/9 (화)', tag:'도착일', fest:false,
    ...
    items:[
      ...
      {time:'16:00', title:'Installation (관람)', note:'Fredericiagade 57, 1310 Copenhagen K — 16:00-16:30 CET', tag:'🏛 전시', color:'#6d3b54', _lat:55.6844, _lng:12.5903},
      {time:'17:30', title:'뉘하운(Nyhavn) 운하 산책', note:'컬러풀 건물과 운하 — 저녁 조명이 아름다운 코펜하겐 상징', tag:'🚶 투어', color:'#3a4a5a', _lat:55.6797, _lng:12.5913},
      {time:'19:30', title:'저녁식사 — Nyhavn 인근', note:'Noma 예약 없다면 운하변 비스트로 추천 · 내일 페스티벌을 위해 일찍 귀숙', tag:'☕ 다이닝', color:'#7a4a1e'},
    ]
  },
  ...
  { dayIdx:3, label:'DAY 3', date:'6/11 (목)', tag:'페스티벌 2일차', fest:true,
    ...
    items:[
      ...
      {time:'11:00', title:'The Power of Daylight (VELUX)', note:'Århusgade 126 — 자연광과 디자인·웰빙의 관계 아티스트 토크', tag:'💬 토크', color:'#2f6b6b', _lat:55.7120, _lng:12.5921},
      ...
      {time:'14:00', title:'Islands Brygge 이동 (메트로/자전거)', note:'M4 Orientkaj → Islands Brygge 환승 또는 자전거 20분', tag:'🚶 투어', color:'#3a4a5a'},
  ...
  { dayIdx:4, label:'DAY 4', date:'6/12 (금)', tag:'페스티벌 3일차', fest:true,
    ...
    items:[
      ...
      {time:'14:00', title:'Christianshavn 이동 — Ege Carpets', note:'Christianshavns Kanal 4 — 운하변 카펫 브랜드 · 크리스티안스하운 시작', tag:'🏛 전시', color:'#6d3b54', _lat:55.6737, _lng:12.5892},
      {time:'15:00', title:'House of Finn Juhl × Edition — Artistry of Craftsmanship', note:'Strandgade 66 — 핀 율 가구 장인 정신 전시 · 페스티벌 최고 하이라이트 중 하나', tag:'🏛 전시', color:'#6d3b54', _lat:55.6732, _lng:12.5888},
      {time:'16:00', title:'Closing Symposium: Make This Moment Matter', note:'페스티벌 클로징 심포지엄 · 참가 가능 시 참석 권장', tag:'💬 토크', color:'#2f6b6b'},
      {time:'저녁', title:'페스티벌 클로징 파티 & 기념 디너', note:'참가 브랜드들의 클로징 이벤트 탐방 · 페스티벌 마지막 밤', tag:'☕ 다이닝', color:'#7a4a1e'},
    ]
  },
  { dayIdx:5, label:'DAY 5', date:'6/13 (토)', tag:'자유 관광 — 코펜하겐 클래식', fest:false,
    theme:'못 본 쇼룸 보충 + 코펜하겐 관광 명소',
    items:[
      ...
      {time:'12:30', title:'점심 — 중앙역 인근', note:'Riz Raz (채식 뷔페) 또는 Mikkeller 펍 · 가벼운 코펜하겐 런치', tag:'☕ 다이닝', color:'#7a4a1e'},
      {time:'14:00', title:'Rundetårn (둥근탑) 전망대', note:'Købmagergade 52A — 나선형 램프 등반 · 코펜하겐 전경', tag:'🚶 투어', color:'#3a4a5a', _lat:55.6814, _lng:12.5757},
      {time:'15:30', title:'Strøget 쇼핑가 — 디자인 숍 투어', note:'Hay, Georg Jensen, Royal Copenhagen 등 · 기념품 쇼핑', tag:'🏛 전시', color:'#6d3b54'},
      {time:'17:00', title:'인어공주 동상 (Little Mermaid)', note:'Langelinie — 코펜하겐 상징 · 항구 산책', tag:'🚶 투어', color:'#3a4a5a', _lat:55.6929, _lng:12.5993},
      {time:'19:00', title:'저녁식사 — Nørrebro 로컬 레스토랑', note:'젊은 감각의 레스토랑 밀집 지역 · Relæ, Manfreds 등 추천', tag:'☕ 다이닝', color:'#7a4a1e'},
    ]
  },
  { dayIdx:6, label:'DAY 6', date:'6/14 (일)', tag:'근교 당일치기 — Louisiana', fest:false,
    theme:'Louisiana 현대미술관 + 해안선 드라이브',
    items:[
      {time:'10:00', title:'코펜하겐 중앙역 → Louisiana 기차 출발', note:'Humlebæk 역 하차 · 약 35분 · InterCity 또는 Lyn 열차', tag:'🚶 투어', color:'#3a4a5a'},
      {time:'10:45', title:'Louisiana 현대미술관 도착', note:'GL Strandvej 13, Humlebæk — 해안절벽 위 건축 · 북유럽 최고 미술관', tag:'🏛 전시', color:'#6d3b54', _lat:55.9695, _lng:12.5430},
      {time:'11:00', title:'Louisiana 관람 — 상설 컬렉션', note:'Giacometti, Calder, Moore 등 거장 조각 · 바다 조망 테라스 산책', tag:'🏛 전시', color:'#6d3b54', _lat:55.9695, _lng:12.5430},
      {time:'13:00', title:'Louisiana 레스토랑 런치', note:'미술관 내 바다 전망 레스토랑 · 덴마크 오픈 샌드위치 추천', tag:'☕ 다이닝', color:'#7a4a1e'},
      {time:'14:30', title:'Louisiana 관람 — 기획전 & 조각공원', note:'야외 조각공원 산책 · 바다와 숲이 어우러진 공간 · 사진 명소', tag:'🏛 전시', color:'#6d3b54'},
      {time:'17:00', title:'코펜하겐 귀환 기차', note:'Humlebæk → 코펜하겐 중앙역 · 저녁 자유 시간', tag:'🚶 투어', color:'#3a4a5a'},
      {time:'19:30', title:'저녁식사 — 베스테르브로 인근', note:'숙소 근처 · Kødbyen (고기 지구) 레스토랑 탐방', tag:'☕ 다이닝', color:'#7a4a1e'},
    ]
  },
  { dayIdx:7, label:'DAY 7', date:'6/15 (월)', tag:'자유 + 정리', fest:false,
    theme:'마지막 코펜하겐 탐방 & 짐 정리',
    items:[
      {time:'10:00', title:'Designmuseum Danmark 여유롭게 재방문', note:'Bredgade 68 — 페스티벌 중 바빴다면 오늘 천천히 감상', tag:'🏛 전시', color:'#6d3b54', _lat:55.6866, _lng:12.5928},
      {time:'12:00', title:'점심 — Bredgade 인근 카페', note:'왕실지구 골목 카페 · 편안한 마지막 코펜하겐 런치', tag:'☕ 다이닝', color:'#7a4a1e'},
      {time:'13:30', title:'Fritz Hansen 플래그십 스토어 & Fredericia', note:'Løvstræde — 구매 결정 못한 디자인 아이템 최종 쇼핑', tag:'🏛 전시', color:'#6d3b54', _lat:55.6795, _lng:12.5773},
      {time:'15:00', title:'Nyhavn 마지막 산책 & 기념사진', note:'운하변 카페에서 커피 · 코펜하겐과 작별 인사', tag:'🚶 투어', color:'#3a4a5a', _lat:55.6797, _lng:12.5913},
      {time:'17:00', title:'숙소 복귀 & 짐 정리', note:'내일 체크아웃 준비 · 구매 물품 정리 · 이른 취침 권장', tag:'🚶 투어', color:'#3a4a5a'},
    ]
  },
];
...
function addRecPin(lat, lng, title, color, tag){
  ...
  const icon = L.divIcon({
    className:'',
    html:`<div class="plan-pin" style="background:${color};box-shadow:0 0 0 3px rgba(155,122,181,.5)"><span>${emoji}</span></div>`,
    iconSize:[30,30], iconAnchor:[15,30], popupAnchor:[0,-32]
  });
...
function renderRecommend(){
  ...
  RECOMMEND.forEach((day)=>{
    ...
    const hasDayItems = day.items.length > 0;
    const allAdded = hasDayItems && day.items.every((_,i)=>recChecked.has(`${day.dayIdx}-${i}`));
    ...
    if(day.items.length===0){
      itemsEl.innerHTML = `<div style="padding:12px 20px;font-size:12px;color:var(--slate);opacity:.6">이 날은 비어있어요. 행사 탭이나 +장소 탭에서 직접 추가하세요.</div>`;
    } else {
      ...
      day.items.forEach((it,i)=>{
        ...
        row.addEventListener('click', ()=>{
          ...
          // 현재 항목 설정
          if(recMapCurr?.row) recMapCurr.row.classList.remove('map-active');
          recMapCurr = {lat:it._lat, lng:it._lng, title:it.title, color:dayColor, tag:it.tag, row};
...
async function applyRecItems(){
  ...
  if(btn){ btn.textContent=`✓ ${added}개 일정에 추가됐어요!`; btn.disabled=true; }
  setTimeout(()=>renderRecommend(), 1500);
}

/* ---------- RENDER: DISTRICTS ---------- */
function renderDist(){
  const el=document.getElementById('scroll'); el.innerHTML='';
  const wrap=document.createElement('div'); wrap.className='dist-list';
  DISTRICTS.forEach(d=>{
    const row=document.createElement('div'); row.className='dist';
    row.innerHTML=`
      <span class="dist-swatch" style="background:${d.color}"></span>
      <div class="dist-info">
        <div class="dist-name">${d.name}</div>
        <div class="dist-desc">${d.desc} · ${d.when}</div>
      </div>
      <span class="dist-go">지도 ▸</span>`;
    row.addEventListener('click',()=>{ map.flyTo([d.lat,d.lng],14.5,{duration:1}); markers[d.key].openPopup(); });
...
function renderInfo(){
  ...
  el.innerHTML=`<div class="info">
    ...
    <p style="font-size:11px;opacity:.7">※ 지도의 ★ 숙소 핀은 Airbnb가 정확 좌표를 공개하지 않아 베스테르브로·호수 인근의 대략적 위치입니다. 체크인 후 정확한 주소로 맞추세요.</p>
    <h3>고정 예약</h3>
    <ul>
...
function sortDayByTime(di){ plan[di].items.sort((a,b)=>timeToMin(a.time)-timeToMin(b.time)); }
...
/* ── 질문 모드: 답변 생성 → 카드에 표시 → 선택적으로 일정 추가 ── */
async function _drawerQuestion(raw,di,ii,item,input,btn,st,resp){
  ...
  const nextItems = plan[di].items.slice(ii+1).filter(it=>it.time).slice(0,3)
    .map(it=>`${it.time} ${it.title}`).join(', ') || '없음';
  const gpsStr = currentPos
    ? `현재 위치: 위도 ${currentPos.lat.toFixed(5)}, 경도 ${currentPos.lng.toFixed(5)}`
    : '현재 위치: 미확인 (지도에서 "내 위치" 버튼을 눌러 활성화 가능)';

  const sys=`You are a knowledgeable and warm travel assistant for a Korean couple (미주 and 상효) visiting Copenhagen for the 3 Days of Design festival, June 8–16, 2026.
Respond in Korean. Be specific, practical, and helpful. You can answer anything about:
- What to do next / recommendations based on location and remaining schedule
- Copenhagen places, food, coffee, transportation, culture
- Festival events, venues, and districts
- Any travel question big or small

If your answer includes a specific place or activity to add to the schedule, append exactly one tag at the very end:
<add>{"title":"장소 또는 활동명","time":"HH:MM","note":"한 줄 설명","day_index":${di}}</add>
Use 24h time. day_index ${di} = ${plan[di]?.date}.
Only add this tag when your recommendation is specific enough to schedule. Omit entirely for general answers.`;

  const ctx=`현재 선택 일정: "${item.title}" (${dayLabel})
${gpsStr}
오늘 전체 일정:
${todayList}
이후 남은 일정: ${nextItems}
...
질문/요청: ${raw}`;
  ...
  try{
    const r=await fetch('/api/extract',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({system:sys,input:ctx,max_tokens:800})});
    ...
    // <add> 파싱
    let suggestion=null;
    const m=text.match(/<add>([\s\S]*?)<\/add>/);
    if(m){ try{ suggestion=JSON.parse(m[1].trim()); }catch(e){} text=text.replace(/<add>[\s\S]*?<\/add>/,'').trim(); }

    // 응답 카드 렌더링
    document.getElementById('drawerRespText').innerHTML=text.replace(/\n/g,'<br>');
    const act=document.getElementById('drawerRespActions');
    act.innerHTML='';

    if(suggestion){
      const addBtn=document.createElement('button');
      addBtn.className='drawer-sugg-btn';
      addBtn.textContent=`＋ "${suggestion.title}" 일정 추가`;
      addBtn.onclick=()=>{
        const td=suggestion.day_index??di;
        if(plan[td]){
          plan[td].items.push({time:suggestion.time||'',title:suggestion.title,note:suggestion.note||'',dist:'',_user:true});
          sortDayByTime(td); savePlan(); renderPlan();
          resp.classList.remove('show');
    ...
    dimBtn.className='drawer-dismiss-btn'; dimBtn.textContent='닫기';
    dimBtn.onclick=()=>resp.classList.remove('show');
    act.appendChild(dimBtn);
...
// SALU 고정 마커
const saluIcon=L.divIcon({className:'',html:`<div class="plan-pin" style="background:${DAY_COLORS[2]}"><span>🍽</span></div>`,iconSize:[30,30],iconAnchor:[15,30],popupAnchor:[0,-32]});
const saluMarker=L.marker([55.665398,12.550298],{icon:saluIcon}).addTo(map);
saluMarker.bindPopup(`<div class="pop-name">🍽 Food & Music with SALU</div><div class="pop-desc">소셜 다이닝 (3명 예약) · 6/10(수) 17:00–20:00<br>Folkehuset Absalon · Sønder Blvd. 73</div>`);
...
// 숙소·SALU
const stayRow=document.createElement('div'); stayRow.className='leg-row';
stayRow.innerHTML=`<span class="leg-dot" style="background:#1a1714">★</span><b>우리 숙소</b>`;
stayRow.style.cssText='border-bottom:1px solid var(--line);padding-bottom:5px;margin-bottom:5px;cursor:pointer';
stayRow.onclick=()=>{map.flyTo([STAY.lat,STAY.lng],15,{duration:1});stayMarker.openPopup()};
...
DAY_COLORS.forEach((color,i)=>{
  ...
  r.innerHTML=`<span class="leg-dot" style="background:${color}">●</span><span style="font-size:10px">${DAY_LABELS[i]}</span>`;
...
distHead.style.cssText='font-family:Space Mono,monospace;font-size:9px;font-weight:700;letter-spacing:.1em;opacity:.5;padding:6px 0 3px;border-top:1px solid var(--line);margin-top:4px';
distHead.textContent='8 DISTRICTS';
leg.appendChild(distHead);
DISTRICTS.forEach((d,i)=>{
  const r=document.createElement('div'); r.className='leg-row'; r.style.cursor='pointer';
  r.innerHTML=`<span class="leg-dot" style="background:${d.color};border-radius:50%"></span><span style="font-size:10px">${i+1}. ${d.name}</span>`;
...
function updateDayProgress(di){
  ...
  const done = items.filter(it=>it._done).length;
...
function showFestMarker(ev, id){
  ...
  const lat = dist.lat + ((h%100)/100-0.5)*0.004;
  const lng = dist.lng + (((h>>8)%100)/100-0.5)*0.006;
  const icon = L.divIcon({
    className:'',
  ...
  const m = L.marker([lat,lng],{icon,zIndexOffset:300}).addTo(map);
  m.bindPopup(`<div class="pop-name">${ev.title}</div><div class="pop-desc">${ev.time} · ${ev.venue}<br><b style="color:${dkColor}">${ev.district}</b></div>`);
  festMarkers[id]=m;
